...
def get_scatter_data_of_quad():
    ...
    while len(x_axis) <= 180:
        ...
        if y < 500:
            x_axis.append(x)
            y_axis.append(y)
    return x_axis + [random.uniform(0, 25) for i in range(20)], \
        y_axis + [random.uniform(0, 500) for i in range(20)]
...
print('二次数据集中有', len(quad_data_set_x) - 1, '个点', '\n')
plt.scatter(quad_data_set_x, quad_data_set_y)
plt.show()

line_data_set_x, line_data_set_y = get_scatter_data_of_line()
print('一次数据集中有', len(quad_data_set_x) - 1, '个点', '\n')
plt.scatter(line_data_set_x, line_data_set_y)
plt.show()
